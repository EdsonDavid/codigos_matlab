function dibujar_barra_deformada_portico(A, E, I, x1,y1, x2,y2, qxloc,qyloc, qe, ae, esc2, esc3, esc4, esc5)
...
xinit = linspace(0, sqrt((x2-x1)^2 + (y2-y1)^2), 50);
...
text(ss(end), aa(end), num2str( qe(4)));
...
text(ss(end), vv(end), num2str(-qe(5)));
...
%% ------------------------------------------------------------------------
   function res = bc(YL,YR)
      ...
      res = [ % tramo 1:   YL: apoyo izquierdo, YR: apoyo derecho
              YL(5,1) - ae(1)          % uloc(0)
              YL(1,1) - ae(2)          % vloc(0)
              YL(2,1) - ae(3)          % thetaloc(0)
              YR(5,1) - ae(4)          % uloc(L)
              YR(1,1) - ae(5)          % vloc(L)
              YR(2,1) - ae(6) ];       % thetaloc(L)
   end
end
%%End
